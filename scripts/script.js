...
module.exports = function(bot){
  ...
  bot.hear(/(bees?|be)/i, function(res){
    return res.send(res.random(beeReactions));
  });

  bot.respond(/hi|hello|howdy/i, function(res){
    return res.reply(res.random(greetings));
  });

  bot.respond(/what is the capital of (.*)/i, function(res){
    var state = res.match[1];

    for (let i = 0; i < capitals.length; i++) {
      if (state == capitals[i].state) {
        return res.send("The capital of " + state + " is " + capitals[i].capital);
      };
    };
  });
  ...
  bot.respond(/(what's)? for (lunch|dinner)/i, function(res){
    var listArr = [];
    for (let i = 0;i < menu.length; i++) {
        listArr.push(menu[i].food);
    };
    var listStr = listArr.join(", ");
    return res.send("Here's what we have on the menu today: " + listStr);
  });
  ...
  bot.respond(/kitty @?([\w .\-]+)\?*$/i, function(res) {
    ...
    if (users.length === 1) {
      ...
      return res.reply(res.random(kittens));
    } else {
      return res.reply("Sorry, I don't think " + name + " is in this channel.");
    };
...
// ]';
// var capitals = JSON.parse(jsonCapitals);
